...
def svg_bar_chart(title, data_pairs, width=1000, height=260, margin=350, rotate_labels=False):
    ...
    for i, (lab, val) in enumerate(data_pairs):
        ...
        svg.append(f'<text x="{margin - 8}" y="{y+bar_height/2}" text-anchor="start" dominant-baseline="middle" font-size="12">{html.escape(str(lab))}</text>')
...
class PisgStats(commands.Cog):
    ...
    async def pstats_html(self, ctx):
        ...
        channels_svg = svg_bar_chart("Kanaalactiviteit (berichten)", ch_pairs, margin=300)
