...
class Prutser(commands.Cog):
    ...
    async def on_member_update(self, before, after):
        ...
        if prutser_role not in before.roles and prutser_role in after.roles:
            ...
            if not assigned_at:
                ...
                await self.log_action(after.guild, f":warning: {after.display_name} heeft handmatig de prutser-rol gekregen.")
    ...
    async def _remove_prutser_later(self, member, prutser_role, duration):
        ...
        if prutser_role in member.roles:
            try:
                await member.remove_roles(prutser_role)
                await self.log_action(member.guild, f":white_check_mark: {member.display_name} is geen prutser meer (timer verlopen, handmatig toegevoegd).")
            except discord.Forbidden:
